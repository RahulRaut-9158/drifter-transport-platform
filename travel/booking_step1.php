...
if (!isset($_SESSION['loggedin'])) {
    header("Location: /Drifter/front/login.php?redirect=/Drifter/travel/booking_step1.php"); exit;
}
$navActive = '';
...
<title>Book Travel Vehicle — Drifter</title>
<style>
.page-hero{
...
.steps-nav{display:flex;justify-content:center;align-items:center;
  gap:0;padding:28px 24px;background:white;box-shadow:0 2px 12px rgba(0,0,0,0.06);}
.step-item{display:flex;flex-direction:column;align-items:center;gap:6px;position:relative;padding:0 32px;}
.step-item:not(:last-child)::after{content:'';position:absolute;right:-1px;top:18px;width:60px;height:2px;background:#e2e8f0;}
.step-circle{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:0.9rem;background:#e2e8f0;color:#94a3b8;transition:all 0.3s;}
...
.form-card{background:white;border-radius:16px;padding:40px;box-shadow:0 4px 24px rgba(0,0,0,0.08);}
.form-card h2{font-size:1.3rem;font-weight:700;margin-bottom:28px;color:var(--text);padding-bottom:16px;border-bottom:2px solid #f1f5f9;}
.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:20px;}
.form-group{display:flex;flex-direction:column;gap:6px;}
.form-group.full{grid-column:span 2;}
.form-group label{font-size:0.82rem;font-weight:600;color:#475569;}
.form-group input{padding:12px 14px;border:2px solid #e2e8f0;border-radius:10px;font-size:0.95rem;transition:all 0.25s;outline:none;font-family:inherit;background:#fafafa;}
.form-group input:focus{border-color:#3a7d44;background:white;box-shadow:0 0 0 3px rgba(168,223,142,0.22);}
...
.user-info{background:#F0FFDF;border:1px solid #A8DF8E;border-radius:10px;padding:12px 16px;margin-bottom:24px;font-size:0.85rem;color:#2d6235;display:flex;align-items:center;gap:8px;}
...
@media(max-width:600px){.form-grid{grid-template-columns:1fr;}.form-group.full{grid-column:span 1;}.steps-nav{padding:20px 8px;}.step-item{padding:0 12px;}}
</style>
</head>
<body>
<div class="page-hero">
  <h1>🚌 Book Travel Vehicle</h1>
  <p>Find comfortable travel vehicles near your pickup location</p>
</div>
<div class="steps-nav">
...
document.getElementById('bookForm').addEventListener('submit', function(e) {
  if (!/^[0-9]{10}$/.test(mob.value.trim())) {
    e.preventDefault(); mob.closest('.form-group').classList.add('has-err'); mob.focus(); return;
  }
