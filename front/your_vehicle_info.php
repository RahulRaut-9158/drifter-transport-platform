...
// Get owner email for legacy owner_name matching
...
$vstmt = $conn->prepare("SELECT * FROM vehicles WHERE (owner_name=? OR owner_name=?) AND vehicle_category='transport' ORDER BY created_at DESC");
$vstmt->bind_param("ss",$username,$ownerEmail); $vstmt->execute();
$vehicles = $vstmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Fetch ALL bookings for this owner's transport vehicles
$allBookings = [];
if (!empty($vehicles)) {
...
<title>Owner Dashboard — Transport — Drifter</title>
<style>
/* HERO */
.page-hero{padding:48px 24px 36px;text-align:center;color:white;position:relative;overflow:hidden;background:#212529;}
.page-hero::after{content:'';position:absolute;inset:0;background:url('https://images.unsplash.com/photo-1519003722824-194d4455a60c?w=1920&q=80') center/cover no-repeat;filter:brightness(0.12);z-index:0;}
...
<div class="page-hero">
  ...
  <div class="hero-tabs">
    <a href="/Drifter/front/your_vehicle_info.php" class="hero-tab active"><i class="fas fa-truck"></i> Transport</a>
    <a href="/Drifter/front/your_vehicle_travel.php" class="hero-tab"><i class="fas fa-bus"></i> Travel</a>
  </div>
</div>
...
<div class="dash-wrap">
  ...
  <div class="success-banner"><i class="fas fa-check-circle"></i> Vehicle registered successfully! You can now receive bookings.</div>
  <?php endif; ?>
  ...
  <div class="section-head" style="margin-top:40px;">
    <h2><i class="fas fa-truck"></i> My Transport Vehicles</h2>
    <a href="/Drifter/transport/add_vehicle.php" class="add-btn"><i class="fas fa-plus"></i> Add Vehicle</a>
  </div>

  <?php if (empty($vehicles)): ?>
    <div class="empty-state">
      <div class="ei">🚚</div>
      <h3>No Transport Vehicles Yet</h3>
      <p>Register your first vehicle to start receiving bookings from customers.</p>
      <a href="/Drifter/transport/add_vehicle.php" class="add-btn"><i class="fas fa-plus"></i> Add Your First Vehicle</a>
    </div>
  <?php else: ?>
    <div class="vehicles-grid">
      ...
      <div class="v-card" id="vcard-<?= $vid ?>">
        ...
        <div class="v-body">
          <div class="v-top">
            <div class="v-name"><?= htmlspecialchars($v['owner_name']) ?>'s Transport</div>
...
function updateBooking(id, action, btn) {
  ...
  fetch('/Drifter/front/api_update_booking.php', {
    ...
    if (d.success) {
      // Update status pill in ALL tabs
